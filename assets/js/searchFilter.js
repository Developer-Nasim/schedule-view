$(document).ready(function(){
    ...
    function HeaderSearchBox() {
        ...
        $(function(){  
            $('#header-search-bar').keydown(function(e){
            $key = e.keyCode;
            if ( $key == 38 || $key == 40 ) {
                nextItem($key);
                return;
            ...
            setTimeout(function() { 
                ...
                if ( $search == '' || ! $('input').val ) {
                $('.header_output').html('').slideUp();
                } else {
                $('.header_output').html($return).slideDown();
                }
            
                $('.prediction-item').on('click', function(){
                $text = $(this).find('span').text();
                $('.header_output').slideUp(function(){
                    $(this).html('');
                });
                $('#header-search-bar').val($text);
                });
        ...
        $('#header-search-bar').focus(function(){
        if ( $('.prediction-item').length > 0 ) {
            $('.header_output').slideDown();
        }
        
        $('#searchform').submit(function(e){
            e.preventDefault();
            $text = $('.focus').find('span').text();
            $('.header_output').slideUp();
            $('#header-search-bar').val($text);
            $('input').blur();
        });
        });
        
        $('#header-search-bar').blur(function(){
            if ( $('.prediction-item').length > 0 ) {
                $('.header_output').slideUp();
            }
        });
